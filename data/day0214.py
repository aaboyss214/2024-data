...
edge_ary.sort(key=lambda data: data[0], reverse=True) ##################이 코드 분석

newAry = []
for i in range(0, len(edge_ary), 2) :
	newAry.append(edge_ary[i])
...
# for a in range(6):
# 	for b in range(6):
# 		sum = sum +G1.graph[a][b]
# print(sum/2)
print(sum(sum(row) for row in G1.graph)/2) #####=> 분석